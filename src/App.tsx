...
function App() {
  ...
  const showMintableAssets = async () => {
    ...
    const response = await contract.current
      .connect(currentAccount!)
      .maxSupply();
    const max_supply = response.toString();
    let owners: any[] = [];
    let count = 0;
      ...
      <header className="App-header">
        ...
        {address ? 
          <>
            ...
            <Button variant="contained" onClick={() => mintAsset(0.001)}> {/* Only 0.1GoreliETH I can get from goerli faucet. Mint only 0.001 ofr testing. */}
              Mint Asset
            </Button>
